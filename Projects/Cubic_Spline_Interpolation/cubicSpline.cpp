...
std::vector<std::vector<double>> cubicSpline(
    ...
    for (std::size_t i = 0; i < x.size()-1; i++){
        ...
        std::vector<double> yeet = linspace(0, x[i+1] - x[i], dif*10 + 1);
        std::vector<double> yote = a[i] + b[i] * (yeet) + c[i] * (yeet)*(yeet) + d[i] * (yeet)*(yeet)*(yeet);

        std::vector<double> x_span = linspace(x[i], x[i+1], dif*10 + 1);

        x_final.insert(x_final.end(), x_span.begin(), x_span.end());
